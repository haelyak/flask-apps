...
# Pig latin page, when we click translate, moves to text result page
@app.route('/lostgrutor',methods=['GET','POST'])
def lostgrutor():
    if request.method == 'POST':
        old_text = request.form['text']
        new_text = pythonfunctions.convert_to_coordinates(old_text)
        return render_template('lostgrutorResults.html', old_text=old_text, new_text=new_text)
    return render_template('lostgrutor.html', title='Home')
